...
@Component({
  selector: 'app-pdv-zone-assign',
  templateUrl: './pdv-zone-assign.component.html',
  styles: [`
    .dragOver {
      background: #12479b;
      color: white
...
export class PdvZoneAssignComponent implements OnInit {

  currentUser: any
  showContents:boolean = false
  mainCredential:string = 'config_zoneAsignPdv'
  ...
  buildAssign(data){
    ...
    for( let item of data ){
      ...
      if( item['customZone'] ){
        if( !result[item['customZone']] ){
          result['none'].push(item)
        }else{
          result[item['customZone']].push(item)
        }
  ...
  getSupList(){
    ...
    this._api.restfulGet( `${this.selectedCountry}`,'Lists/pdvZoneList' )
              .subscribe( res => {
  ...
  getModules(){
    ...
    this._api.restfulGet( `${this.selectedCountry}/${moment().format('YYYY-MM-DD')}`,'Lists/pdvModuleList' )
              .subscribe( res => {
  ...
  assign(id, sup, callback){
    ...
    this._api.restfulPut( params,'Config/pdvZoneAssign' )
              .subscribe( res => {
