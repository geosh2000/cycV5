...
@Component({
  selector: 'app-sum-asesor-chart',
  templateUrl: './sum-asesor-chart.component.html',
  styles: [],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SumAsesorChartComponent implements AfterViewInit, OnChanges {
  ...
  constructor() {
    ...
    this.options = {
                    ...
                    title: {
                        text: 'Avance de venta por Asesor'
                    },
                    subtitle: {
                        text: 'corte al...'
                    },
  ...
  setData(){
    ...
    // console.log(this)
    if( this.chart['sumAsesor'] && this.data.length > 0 ){
      ...
      for( let i of this.data ){
        if(i['Supervisor'] == this.filterZone){
          supers.push(i['Nombre'])
          metaT.push(parseInt(i['meta_asesor']))
          metaTD.push(Math.round(i['meta_asesor_diaria']))
          metaH.push(parseInt(i['MetaHotel_asesor']))
          metaHD.push(Math.round(i['MetaHotel_asesor_diaria']))
      ...
      this.chart['sumAsesor'].xAxis[0].setCategories(supers)
      this.chart['sumAsesor']['series'][0].update( { data: metaT } )
      this.chart['sumAsesor']['series'][1].update( { data: metaTD } )
      this.chart['sumAsesor']['series'][2].update( { data: montoT } )
      this.chart['sumAsesor']['series'][3].update( { data: metaH } )
      this.chart['sumAsesor']['series'][4].update( { data: metaHD } )
      this.chart['sumAsesor']['series'][5].update( { data: montoH } )

      this.chart['sumAsesor'].title.update({ text: `Avance por Asesor (Supervisor ${this.filterZone}) || ${ this.months[parseInt(this.mes)] } - ${this.anio}`})
      this.chart['sumAsesor'].subtitle.update({ text: `Corte al ${ parseInt(moment().format('MM')) == mes && parseInt(moment().format('YYYY')) == anio ? moment().format('DD MMM \'YY') : moment(`${anio}-${mes}-01`).add(1, 'month').subtract(1,'days').format('DD MMM \'YY') }`})
    ...
    if(this.parentDiv) {
      ...
      if( this.chart ){
        ...
        for( let group in this.chart ){
          ...
          this.chart['sumAsesor'].setSize(this.divWidth, h);
        }
      }
    }
  }
