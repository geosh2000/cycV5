...
@Component({
  selector: 'app-asesor-pdv-chart',
  templateUrl: './asesor-pdv-chart.component.html',
  styles: [],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class AsesorPdvChartComponent implements AfterViewInit, OnChanges {
  ...
  setData(){
    ...
    // console.log(this)
    if( this.chart['daily'] && this.data.length > 0 ){
      ...
      for( let i of this.data ){
        if(i['Nombre'] == this.filterZone){
          supers.push(i['Fecha'])
          metaT.push(parseInt(i['metaTotal']))
          metaH.push(parseInt(i['metaHotel']))
      ...
      this.chart['daily'].xAxis[0].setCategories(supers)
      this.chart['daily']['series'][0].update( { data: metaT } )
      this.chart['daily']['series'][1].update( { data: montoT } )
      this.chart['daily']['series'][2].update( { data: metaH } )
      this.chart['daily']['series'][3].update( { data: montoH } )

      this.chart['daily'].title.update({ text: `Avance por Dia (Asesor ${this.filterZone}) || ${ this.months[parseInt(this.mes)] } - ${this.anio}`})
      this.chart['daily'].subtitle.update({ text: `Corte al ${ parseInt(moment().format('MM')) == mes && parseInt(moment().format('YYYY')) == anio ? moment().format('DD MMM \'YY') : moment(`${anio}-${mes}-01`).add(1, 'month').subtract(1,'days').format('DD MMM \'YY') }`})
    ...
    if(this.parentDiv) {
      ...
      if( this.chart ){
        ...
        for( let group in this.chart ){
          ...
          this.chart['daily'].setSize(this.divWidth, h);
        }
      }
    }
  }
